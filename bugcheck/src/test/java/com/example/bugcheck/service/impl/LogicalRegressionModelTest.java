...
class LogicalRegressionModelTest {
    double[][] info = new double[997][62];

    @Test
    void getResult() {
        double[] array_x = new double[62];
        for(int i=0; i<62; i++)
            array_x[i] = i+5;
        LogicalRegressionModel logicalRegressionModel = new LogicalRegressionModel();
        int a = logicalRegressionModel.getResult(array_x);
        assertEquals(1,a);
    }
    ...
    void updateW() {
        LogicalRegressionModel logicalRegressionModel = new LogicalRegressionModel();
        logicalRegressionModel.updateW(info);
    }

    @Test
    void getAccuracy() {
        LogicalRegressionModel logicalRegressionModel = new LogicalRegressionModel();
        if(logicalRegressionModel.getAccuracy(info) > 0.7)
